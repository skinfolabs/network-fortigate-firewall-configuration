...
Inspect supported outbound file transfers with FortiGate antivirus and validate the block using a safe security test sample rather than live malware.
...
| Step | Description |
|------|-------------|
| Create the flow-based antivirus profile | The Office AV Profile profile is set to flow based inspection and scanning is enabled for the listed web, mail, and file transfer protocols. |
| Apply antivirus to outbound traffic | Office AV Profile is selected on the outbound office policy. |
| Test with a safe security sample | The workstation requests a safe WildFire test sample and receives a high security alert instead of the file. |
| Confirm the antivirus event in logs | The Antivirus log is opened after the client test and the blocked sample events are reviewed. |
